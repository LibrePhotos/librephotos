...
All the machine learning models live here: image captioning (Florence-2, Moondream 2), the CLIP model used for semantic search, tagging (MobileCLIP-S2, SigLIP 2), the optional LLM (Mistral) and the InsightFace face recognition models. Rather than being baked into the image, they are downloaded by the "Download Models" job, or when you pick a new model in the `Admin Area` (for example the `Captioning Model` setting). Because of that, this folder only grows as you enable features, but it is usually by far the largest folder in `/protected_media` — several GB in total.
