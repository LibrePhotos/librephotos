import { IconScreenshot as Screenshot } from "@tabler/icons-react";
...
export function OnlyScreenshots() {
  ...
  const { data: photosGroupedByDate, isLoading } = useFetchDateAlbumsQuery({ photosetType: Photoset.SCREENSHOTS });
  ...
  return (
    <PhotoListView
      title={t("photos.screenshots")}
      loading={isLoading}
      icon={<Screenshot size={50} />}
      photoset={photosGroupedByDate ?? []}
      updateGroups={getAlbums}
      idx2hash={photosFlat}
      selectable
      emptyStateConfig={emptyStateConfig}
      photosetQuery={{ is_screenshot: true }}
    />
  );
}
