...
export function OnlyVideos() {
  ...
  const { data: photosGroupedByDate, isLoading } = useFetchDateAlbumsQuery({ photosetType: PhotosetType.VIDEOS });
  ...
  useFetchDateAlbumQuery(
    { album_date_id: group.id, page: group.page, photosetType: PhotosetType.VIDEOS },
    { skip: !group.id }
  );
