...
import {Checkbox, Popup, Input, Item, Menu, Image, Icon, Header, Container, Divider, Button, Label, Loader, Sticky, Accordion} from 'semantic-ui-react'
...
import {deleteFaces, setFacesPersonLabel, loadFaceToLabel, trainFaces, clusterFaces, fetchInferredFacesList, fetchLabeledFacesList, fetchFacesList} from '../actions/facesActions';
...
class ModalPersonEdit extends Component {
	...
	render() {
			if (this.state.newPersonName.length > 0) {
					var filteredPeopleList = this.props.people.filter((el)=>fuzzy_match(el.text.toLowerCase(),this.state.newPersonName.toLowerCase()))
			} else {
			...
			return (
					<Modal
							ariaHideApp={false}
							onAfterOpen={()=>{this.props.dispatch(fetchPeople())}}
							...
							<div style={{paddingLeft:10,paddingTop:10,overflowY:'scroll',height:window.innerHeight-300-100,width:'100%'}}>
											<div style={{paddingRight:5}}>
															<Header as='h4'>
																	...
																	<Popup 
																			inverted
																			content={'Album "'+this.state.newPersonName.trim()+'" already exists.'}
																			position='bottom center'
																			open={this.props.people.map((el)=>el.text.toLowerCase().trim()).includes(this.state.newPersonName.toLowerCase().trim())}
																			trigger={
																					<Input 
																							fluid
																							error={
																									this.props.people.map((el)=>el.text.toLowerCase().trim()).includes(this.state.newPersonName.toLowerCase().trim())
																							}
																							onChange={(e,v)=>{
																									this.setState({newPersonName:v.value})
																							}}
																							placeholder='Person name' action>
																							<input/>
																							<Button 
																									positive
																									onClick={()=>{
																									...
																									disabled={this.props.people.map((el)=>el.text.toLowerCase().trim()).includes(this.state.newPersonName.toLowerCase().trim())}
																									type='submit'>
																									Add Person
																							</Button>
																					</Input>
											...
											{ filteredPeopleList.length > 0 &&
													filteredPeopleList.map((item)=>{
															return (
																<div style={{height:70,justifyContent:'center',alignItems:'center'}}>
																	<Header
																		as='h4'
																		onClick={()=>{
...
export class FaceDashboard extends Component {
  ...
  static getDerivedStateFromProps(nextProps,prevState){
      ...
      const inferredCellContents = calculateFaceGridCells(inferredGroupedByPersonList,prevState.numEntrySquaresPerRow).cellContents
      const labeledCellContents = calculateFaceGridCells(labeledGroupedByPersonList,prevState.numEntrySquaresPerRow).cellContents
      const nextState = {...prevState,inferredCellContents,labeledCellContents,inferredGroupedByPersonList,labeledGroupedByPersonList}
      return nextState
  }
  ...
  handleResize() {
    ...
    if (this.state.inferredGroupedByPersonList) {
    	const inferredCellContents = calculateFaceGridCells(this.state.inferredGroupedByPersonList,numEntrySquaresPerRow).cellContents
	    this.setState({
	      inferredCellContents,
	    })
    } 
    if (this.state.labeledGroupedByPersonList) {
    	const labeledCellContents = calculateFaceGridCells(this.state.labeledGroupedByPersonList,numEntrySquaresPerRow).cellContents
	    this.setState({
	      labeledCellContents,
	    })
    } 
  ...
  cellRenderer = ({ columnIndex, key, rowIndex, style }) => {
		...
		if (cell) {
			if (!cell.image) {
				...
				const labelProbabilityColor = labelProbability > 0.9 ? 'green' : 
																			labelProbability > 0.8 ? 'yellow' :
																			labelProbability > 0.7 ? 'orange' : 'red'
				...
				var showPhotoIcon = (
					<div style={{left:6,bottom:6,position:'absolute'}}>
					  <Popup 
                            ...
                            inverted
							position="bottom center"
							content={<Image size='large' src={serverAddress+'/media/thumbnails_big/'+cell.photo+'.jpg'}/>}/>
				...
				if (this.state.isScrollingFast) {
					return (
						<div key={key} style={{...style,padding:5}}>
							<Image rounded src={'/thumbnail_placeholder.png'} height={this.state.entrySquareSize-10} width={this.state.entrySquareSize-10}/>
						</div>
					)
				} else {
					if (this.state.selectMode) {
						...
						return (
							<div key={key} style={{...style,padding:5}}>
								<div style={{padding:10, backgroundColor:isSelected ? '#AED6F1' : '#eeeeee'}}>
								<Image
									rounded 
									onClick={()=>{
		...
		return (
	        ...
	        <Menu pointing secondary>
	          ...
	          <Menu.Item
				...
				<div style={{marginLeft:-5,paddingLeft:5,paddingRight:5,height:40,paddingTop:4,backgroundColor:this.state.selectMode?'#AED6F1':'#eeeeee'}}>

				    <Checkbox label={`${this.state.selectedFaces.length} selected`} style={{padding:5}} toggle checked={this.state.selectMode} onClick={()=>{this.setState({selectMode:!this.state.selectMode})}}/>
	                ...
	                <Grid
										...
										onSectionRendered={({rowStartIndex})=>{
											if (activeItem === 'labeled'){
												this.setState({topRowPersonName:this.state.labeledCellContents[rowStartIndex][0].person_name})
											} else {
												this.setState({topRowPersonName:this.state.inferredCellContents[rowStartIndex][0].person_name})												
											}
	                  ...
	                  rowCount={activeItem==='labeled' ? this.state.labeledCellContents.length : this.state.inferredCellContents.length }
	                  width={width}
	                />
	              )}
	            </AutoSizer>  
