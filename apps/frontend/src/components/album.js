...
export class AlbumLocationMap extends Component {
  render() {
    ...
    var markers = photosWithGPS.map(function (photo) {
      return (
        <Marker position={[photo.exif_gps_lat, photo.exif_gps_lon]}></Marker>
      );
    });
    console.log(markers);
    ...
    return (
      <Card>
        ...
        <Card.Content>
          ...
          <Card.Meta>{this.props.person.face_count} Photos</Card.Meta>
        </Card.Content>
      </Card>
    );
  }
...
export class AlbumPeopleGallery extends Component {
  ...
  componentWillReceiveProps() {
    console.log(
      ...
      this.props.fetchingAlbumsPeople,
      this.props.match.params.albumID in this.props.albumsPeople
    );
  }
  ...
  render() {
    ...
    if (this.props.match.params.albumID in this.props.albumsPeople) {
      ...
      return (
        <div>
          <Header as="h1">{album_name}</Header>
          <Divider />
...
export class AlbumAutoGallery extends Component {
  render() {
    ...
    return (
      ...
      >
        <Header as="h1">{album[0].title}</Header>
        {mappedPeopleIcons}
        <Divider />
  ...
  render() {
    ...
    if (this.props.people.length > 0) {
      var mappedPeopleIcons = this.props.people.map(function (person) {
        return (
          <Label
            key={"auto-album-card-" + album_id + "-person-label-" + person.id}
            as="a"
            image
          >
            <img src={serverAddress + person.face_url} />
    ...
    return (
      <Card key={this.props.key}>
        <LazyLoad
          once
          height={150}
          placeholder={<Image src={"https://placehold.jp/150x150.png"} />}
        >
          <Image
            as={Link}
        ...
        <Card.Content>
          ...
          <Card.Meta>
            ...
            <br />
            {this.props.photoCount} Photos {peoplePopup}
            <div
              style={{
                textAlign: "right",
                position: "absolute",
...
export class AlbumAutoCardPlain extends Component {
  ...
  render() {
    ...
    if (this.props.album.people.length > 0) {
      ...
      mappedPeopleIcons = <Image.Group>{mappedPeopleIcons}</Image.Group>;
    } else {
      // empty placeholder so the extra portion (with face icons) of the cards line up
          ...
          <div style={{ padding: "10px" }}>
            ...
            <div style={{ paddingTop: "5px" }}>
              <span style={{ color: "grey" }}>
                {this.props.album.timestamp.split("T")[0]}
              </span>
            </div>
            <div>
  ...
  render() {
    ...
    if (this.props.people.length > 0) {
      var mappedPeopleIcons = this.props.people.map(function (person) {
        return (
          <Label
            key={"date-album-card-" + album_id + "-person-label-" + person.id}
            as="a"
            image
          >
            <img src={serverAddress + person.face_url} />
