...
function Pig({
  ...
  const handleClickFunc = useRef<(event: React.MouseEvent, item: ImageItem) => void>(propHandleClick || defaultHandleClick);
  ...
  const setRenderedItemsFunc = useCallback((imageData: (ImageItem | GroupedImageItem)[]) => {
  ...
  const onScroll = useCallback(() => {
    ...
    window.requestAnimationFrame(() => {
      ...
      const speed = getScrollSpeed(latestYOffsetRef.current, scrollThrottleMs, (speed: string) => {
        setScrollSpeed(speed); // scroll idle callback
      });
  ...
  useEffect(() => {
    ...
    propGetUrl, propHandleClick, propHandleSelection, selectable,
    numberOfItems, imageData, propUpdateGroups, propUpdateItems,
    defaultHandleClick, defaultHandleSelection
  ]);

  // Render methods
  const renderTile = useCallback((item: ImageItem) => {
    ...
    useLqip, thumbnailSize, toprightoverlay, bottomleftoverlay,
    settings, selectedItems, activeTileUrl, scrollSpeed, propSelectedItems
  ]);

  const renderGroup = useCallback((group: GroupedImageItem) => {
        ...
        return (
          <React.Fragment key={key}>
            {settings.groupByDate 
              ? renderGroup(item as GroupedImageItem) 
              : renderFlat(item as ImageItem)}
          </React.Fragment>
        );
      })}
    </div>
