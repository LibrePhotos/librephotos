...
export function AlbumSlugSection({ albumID, album, isPublic, showSettings, refetch }: Props) {
  ...
  const hasChanges = useMemo(() => {
    ...
    const expChanged = expiresDirty && (effectiveExpires || "") !== ((album?.public_expires_at as unknown as string) || "");
  ...
  const onSave = useCallback(() => {
    ...
    toggleAlbumPublic.mutate(
      { albumId: albumID, public: true, slug: (slugDirty ? effectiveSlug.trim() : undefined), expires_at: (expiresDirty ? (effectiveExpires || null) : undefined) },
      {
        onSuccess: () => {
          refetch();
          setSlugDirty(false);
      ...
      {showSettings && (
        <Stack mt="xs" gap="xs">
          <SlugSetting
            ...
            onChange={(v) => { setCustomSlug(v.toLowerCase()); setSlugDirty(true); }}
            isPublic={isPublic}
            albumId={album?.id}
            onMetaChange={(meta) => { setSlugValid(meta.valid); setSlugAvailable(meta.available); setIsCheckingSlug(meta.checking); }}
          />
          <ExpiresSetting
            value={expiresDirty ? expiresAt : effectiveExpires}
            onChange={(v) => { setExpiresAt(v); setExpiresDirty(true); }}
          />
          <Group>
            <Button variant="default" disabled={!canSave} onClick={onSave}>Save link settings</Button>
          </Group>
...
function SlugSetting({ value, onChange, isPublic, albumId, onMetaChange }: SlugProps) {
  ...
  return (
    <TextInput
      ...
      error={!valid ? "Use lowercase letters, numbers and hyphens (3-64)." : (!available ? "This URL is already taken." : undefined)}
