...
def get_location_timeline_orm(user):
    photos = Photo.objects.filter(exif_timestamp__isnull=False, owner=user).order_by(
        "exif_timestamp"
    )
    photo_data = [
        {
            "location": photo.geolocation_json["features"][-1]["text"],
            "exif_timestamp": photo.exif_timestamp,
        }
        for photo in photos
        if photo.geolocation_json["features"][-1]["text"] is not None
    ]

    partitioned = []
    unique_order = 1
    for data in photo_data:
        if partitioned and data["location"] == partitioned[-1]["location"]:
            unique_order += 1
        else:
            unique_order = 1
        data["unique_order"] = unique_order
        partitioned.append(data)

    grouped = {}
    for data in partitioned:
        location = data["location"]
        if location not in grouped:
            grouped[location] = {
                "begin": data["exif_timestamp"],
                "end": data["exif_timestamp"],
            }
        else:
            grouped[location]["end"] = data["exif_timestamp"]

    coalesced = []
    sorted_grouped = sorted(grouped.items(), key=lambda x: x[1]["begin"])
    for i in range(len(sorted_grouped)):
        location, group = sorted_grouped[i]
        begin = group["begin"]
        end = group["end"]
        if i + 1 < len(sorted_grouped):
            end_coalesce = sorted_grouped[i + 1][1]["begin"]
        else:
            end_coalesce = end
        coalesced.append(
            {"location": location, "begin": begin, "end_coalesce": end_coalesce}
        )

    city_start_end_duration = coalesced
    colors = sns.color_palette("Paired", len(city_start_end_duration)).as_hex()
    ...
    for idx, sted in enumerate(city_start_end_duration):
        data.append(
            {
                "data": [sted["end_coalesce"].timestamp() - sted["begin"].timestamp()],
                "color": colors[idx],
                "loc": sted["location"],
                "start": sted["begin"].timestamp(),
                "end": sted["end_coalesce"].timestamp(),
            }
        )
    return data
