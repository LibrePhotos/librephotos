    ...
    try:
        ...
        with open(path, "rb") as image:
            with mmap(image.fileno(), 0, access=ACCESS_READ) as mm:
                return (
                    _locate_google_embedded_video(mm) != -1
                    or _locate_samsung_embedded_video(mm) != -1
                )
...
def _create_embedded_live_photo_stack(
    ...
    existing_stack = photo.stacks.filter(
    ...
    logger.info(
        f"Created Live Photo stack {stack.id} for embedded motion in {image_path}"
    )
    return stack


def _create_apple_live_photo_stack(
    photo: "Photo", video_path: str, user: "User"
) -> PhotoStack | None:
    ...
    if (
        video_photo
        and not video_photo.stacks.filter(
            stack_type=PhotoStack.StackType.LIVE_PHOTO
        ).exists()
    ):
