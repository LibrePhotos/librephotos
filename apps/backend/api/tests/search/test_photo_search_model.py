...
(``photo.search_instance`` / ``photo.caption_instance``) that replaced the old
``Photo.search_captions`` / ``search_location`` properties.
"""
...
class PhotoSearchDirectAccessTest(TestCase):
    """Post-refactor smoke tests: PhotoSearch reached via the reverse accessors."""

    def setUp(self):
        self.user = create_test_user()

    def test_geolocate_updates_search_location(self):
        """update_search_location joins every feature into one comma string."""
    ...
    def test_direct_access_consistency(self):
        """Saved data is readable back through photo.search_instance/caption_instance."""
        photo = create_test_photo(owner=self.user)
