"""
Comprehensive tests for duplicate detection algorithms.

Merged from test_duplicate_detection_logic.py and
test_bktree_and_duplicate_detection.py.

Tests cover:
- BKTree: Burkhard-Keller Tree for efficient Hamming distance queries
  (both character-wise and bitwise distance functions)
...
- Multi-user isolation of detection and pending-duplicate clearing
...
class BKTreeTestCase(TestCase):
    ...
    def test_search_no_matches_returns_empty(self):
        """Test searching a populated tree with nothing in range."""
    ...
    def test_bitwise_hamming_distance_search(self):
        """Test BK-Tree with a bitwise hamming distance function."""
...
class DetectExactCopiesTestCase(TestCase):
    ...
    def test_detects_duplicate_image_hash(self):
        ...
        self.assertEqual(
            duplicates.first().duplicate_type, Duplicate.DuplicateType.EXACT_COPY
        )
...
class DetectVisualDuplicatesTestCase(TestCase):
    ...
    def test_skips_trashed_photos(self):
        """Test trashed photos are excluded from visual detection."""
        self._create_photo_with_phash("aaaaaaaaaaaaaaaa", in_trashcan=True)
        self._create_photo_with_phash("aaaaaaaaaaaaaaaa")
    ...
    def test_creates_visual_duplicate_type(self):
        ...
        self.assertEqual(
            duplicate.duplicate_type, Duplicate.DuplicateType.VISUAL_DUPLICATE
        )
...
class BatchDetectDuplicatesTestCase(TestCase):
    ...
    def test_none_options_runs_without_error(self):
        """Test batch detection with an explicit None options argument."""
        batch_detect_duplicates(self.user, None)

    def test_empty_options_runs_without_error(self):
        """Test batch detection with an empty options dict."""
        batch_detect_duplicates(self.user, {})
...
class MultiUserDuplicateIsolationTestCase(TestCase):
    ...
    def test_detection_does_not_create_duplicates_for_other_users(self):
        """Test that detection never creates Duplicate rows for another user."""
        # Create photos for user1 with same image_hash
        ...
        self.assertEqual(Duplicate.objects.filter(owner=self.user2).count(), 0)
