"""Characterization tests for the ``createuser`` management command.

These pin the CURRENT observable behavior of
``api/management/commands/createuser.py`` -> ``Command.handle`` before any
refactor. They assert what the code does today, not what it arguably should do.
"""
