...
class ThumbnailMigrationTest(TestCase):
    ...
    def test_file_renaming_with_mocked_filesystem(self, mock_media_root):
        ...
        for thumb_dir in ['thumbnails_big', 'square_thumbnails', 'square_thumbnails_small']:
            os.makedirs(os.path.join(self.test_media_root, thumb_dir), exist_ok=True)
        
        # Create dummy "old" thumbnail files with UUID names
        old_files = []
        for thumb_dir in ['thumbnails_big', 'square_thumbnails', 'square_thumbnails_small']:
            old_file = os.path.join(self.test_media_root, thumb_dir, f"{photo_uuid}.webp")
        ...
        for thumb_dir in ['thumbnails_big', 'square_thumbnails', 'square_thumbnails_small']:
            old_path = os.path.join(self.test_media_root, thumb_dir, f"{photo_uuid}.webp")
            new_path = os.path.join(self.test_media_root, thumb_dir, f"{photo_hash}.webp")
        ...
        for thumb_dir in ['thumbnails_big', 'square_thumbnails', 'square_thumbnails_small']:
            ...
            self.assertFalse(os.path.exists(old_path), f"Old file should not exist: {old_path}")
            self.assertTrue(os.path.exists(new_path), f"New file should exist: {new_path}")
    ...
    def test_bulk_update_performance(self):
        ...
        for i, photo in enumerate(photos):
            ...
            self.assertTrue(photo.thumbnail.thumbnail_big.startswith('updated_'))
