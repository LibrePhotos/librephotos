"""
Comprehensive tests for Burst Detection Rules engine and filename patterns.
...
- check_filename_pattern() over DB-backed photos (grouping by directory +
  base name, case insensitivity)
...
class GroupPhotosByTimestampDbTestCase(TestCase):
    """DB-backed timestamp grouping cases not covered by the mock-based suite."""
    ...
    def test_photos_without_timestamp(self):
        ...
        ordered = Photo.objects.filter(pk__in=[p.pk for p in photos]).order_by(
            "exif_timestamp"
        )
