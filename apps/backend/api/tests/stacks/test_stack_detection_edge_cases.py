...
during scan (Photo.files ManyToMany field), not as stacks. Tests for the
removed RAW+JPEG and Live Photo detection functions have been dropped.
...
4. Burst rule parsing/validation and user-stored rules
5. File variant handling edge cases
6. Batch detection option handling and multi-user isolation
7. Memory/performance edge cases
"""
...
class BurstRuleParsingTestCase(TestCase):
    ...
    def test_default_rules_are_valid(self):
        ...
        for rule_dict in default_rules:
            ...
            self.assertIn(
                rule.category, [BurstRuleCategory.HARD, BurstRuleCategory.SOFT]
            )
...
class BatchDetectionSmokeTestCase(TestCase):
    """Smoke tests for batch detection option handling."""
...
class MultiUserDetectionIsolationTestCase(TestCase):
    ...
    def test_clearing_stacks_only_affects_own(self):
        ...
        user1_stacks = PhotoStack.objects.filter(
            owner=self.user1, stack_type=PhotoStack.StackType.BURST_SEQUENCE
        )
        self.assertEqual(user1_stacks.count(), 0)

        # User2 should still have their stack
        user2_stacks = PhotoStack.objects.filter(
            owner=self.user2, stack_type=PhotoStack.StackType.BURST_SEQUENCE
        )
