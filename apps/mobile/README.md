...
The official LibrePhotos mobile app, built on **Expo (SDK 57, New
Architecture)** with an **offline-first, device-mirrored** data model. It
replaced the original bare React Native 0.72 + NativeBase app, which lived at
this path until it was removed; `git log` before the rename still has it.
...
- **Single `@types/react`**. React types must resolve to **one** copy across the
  workspace (apps/mobile and api-client both pin `~19.2.x`). A duplicate
...
- **Jest module mappers use `require.resolve`**. `jest.config.js` maps some
  ...
  that style.
- **React is pinned to `19.2.3`** in `apps/mobile` *and* at the workspace
...
- **`babel-preset-expo` is a direct devDependency** even though `expo` already
  ...
  relative to `apps/mobile/babel.config.js` — cannot find it, and every jest
...
The Android package **deliberately matches the original React Native app**
(`com.librephotosmobile`) so this app ships as an **in-place Play Store update**
...
the first store upload; afterwards it is immovable. The original iOS project
