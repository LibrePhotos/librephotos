...
export async function fetchPhotoMetadata(
  ...
  return parseWithNotification(
    PhotoMetadata,
    response,
    'Failed to parse photo metadata',
  )
}
...
export async function updatePhotoMetadata(
  ...
  return parseWithNotification(
    PhotoMetadata,
    response,
    'Failed to parse updated photo metadata',
  )
}
...
export async function fetchMetadataHistory(
  ...
  const response = await fetchClient.get(
    `/photos/${photoId}/metadata/history?page=${page}&page_size=${pageSize}`,
  )
  return parseWithNotification(
    MetadataHistoryResponse,
    response,
    'Failed to parse metadata history',
  )
}
 ...
 */
export async function revertMetadataEdit(
  photoId: string,
  editId: number,
): Promise<PhotoMetadata> {
  ...
  return parseWithNotification(
    PhotoMetadata,
    response,
    'Failed to parse reverted photo metadata',
  )
}
 ...
 * @param photoId Photo UUID or image_hash
 */
export async function revertAllMetadataEdits(
  photoId: string,
): Promise<PhotoMetadata> {
  const response = await fetchClient.post(
...
export async function fetchBulkMetadata(
  ...
  const response = await fetchClient.get(
    `/photos/metadata/bulk?photo_ids=${photoIds.join(',')}`,
  )
  return parseWithNotification(
