...
export type IncompletePersonFaceListResponse = z.infer<
...
export type IncompletePersonFaceListRequest = z.infer<
...
const fetchIncompleteFaces = (params: IncompletePersonFaceListRequest) => {
  ...
  const url = `/faces/incomplete/?inferred=${inferred}${`&order_by=${orderBy}`}${
    inferred ? `&analysis_method=${method}` : ''
  }${minConfidence ? `&min_confidence=${minConfidence}` : ''}`

  return fetchClient
    .get<IncompletePersonFaceListResponse>(url)
    .then(response => {
...
export const useFetchIncompleteFacesQuery = (
  request: IncompletePersonFaceListRequest,
) =>
  useQuery({
