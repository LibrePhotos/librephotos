...
export const stackKeys = {
  ...
  list: (filters: Record<string, unknown>) =>
    [...stackKeys.lists(), filters] as const,
...
async function fetchStacks(params?: {
  ...
  if (params?.stack_type) {
    queryParams.append('stack_type', params.stack_type)
  }
  if (params?.ordering) {
...
async function fetchStackStats(): Promise<StackStatsResponse> {
  ...
  return parseWithNotification(
    StackStatsResponseSchema,
    response,
    'Failed to parse stack stats',
  )
}

async function addToStack(
  stackId: string,
  data: AddToStackRequest,
): Promise<AddToStackResponse> {
  const response = await fetchClient.post<AddToStackResponse>(
    `/stacks/${stackId}/add/`,
    {
  ...
  return parseWithNotification(
    AddToStackResponseSchema,
    response,
    'Failed to parse add to stack response',
  )
}

async function removeFromStack(
  stackId: string,
  data: RemoveFromStackRequest,
): Promise<RemoveFromStackResponse> {
  const response = await fetchClient.post<RemoveFromStackResponse>(
    `/stacks/${stackId}/remove/`,
    {
      photo_hashes: data.photo_hashes,
    },
  )
  return parseWithNotification(
    RemoveFromStackResponseSchema,
    response,
    'Failed to parse remove from stack response',
  )
}

async function mergeStacks(
  data: MergeStacksRequest,
): Promise<MergeStacksResponse> {
  const response = await fetchClient.post<MergeStacksResponse>(
    '/stacks/merge/',
    data,
  )
  return parseWithNotification(
    MergeStacksResponseSchema,
    response,
    'Failed to parse merge stacks response',
  )
}

async function createManualStack(
  data: CreateManualStackRequest,
): Promise<CreateManualStackResponse> {
  const response = await fetchClient.post<CreateManualStackResponse>(
    '/stacks/manual/',
    data,
  )
  return parseWithNotification(
...
async function detectStacks(
  options?: DetectStacksRequest,
): Promise<DetectStacksResponse> {
  const response = await fetchClient.post<DetectStacksResponse>(
    '/stacks/detect/',
    options || {},
  )
  return parseWithNotification(
    DetectStacksResponseSchema,
    response,
    'Failed to parse detect stacks response',
  )
}
...
async function setCoverPhoto(
  ...
  const response = await fetchClient.post<SetPrimaryResponse>(
    `/stacks/${stackId}/primary/`,
    {
...
// Query hooks
export function useStacksQuery(params?: {
  page?: number
  page_size?: number
  stack_type?: string
  ordering?: string
...
export function useAddToStackMutation() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      stackId: string
      data: AddToStackRequest
    }) => addToStack(stackId, data),
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({
        queryKey: stackKeys.detail(variables.stackId),
      })
