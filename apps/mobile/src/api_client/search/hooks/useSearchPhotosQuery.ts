...
export const useSearchPhotosQuery = (searchTerm: string) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await fetchClient.get<typeof SearchPhotos>(
        `/photos/searchlist/?search=${searchTerm}`,
      )
      ...
      if (currentUser?.semantic_search_topk) {
        const parsed = parseWithNotification(
          SemanticSearchPhotos,
          response,
          'Failed to parse semantic search photos',
        )
        return {
