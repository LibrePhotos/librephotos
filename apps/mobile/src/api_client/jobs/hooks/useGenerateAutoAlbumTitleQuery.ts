...
export const GenerateAutoAlbumTitleQueryKeys = [
  'generateAutoAlbumTitle',
] as const

export type GenerateEventAlbumsTitlesResponse = z.infer<
  typeof GenerateEventAlbumsTitlesResponse
>
  ...
  useQuery({
    ...
    queryFn: async () => {
      const response = await fetchClient.get(
        `/generateeventalbums/titles/?title=${title}`,
      )
