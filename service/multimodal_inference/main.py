...
def generate_tags():
    ...
    try:
        ...
        tagging_model = data.get("tagging_model", "siglip2")
    ...
    try:
        if tagging_model != "siglip2":
            print(
                f"multimodal inference: Unsupported legacy tagging model '{tagging_model}', using siglip2"
            )

        result = get_siglip2_instance().predict(image_path, threshold=0.05, max_tags=10)
